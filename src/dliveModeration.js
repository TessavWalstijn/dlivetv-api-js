...
class dliveMod {
  ...
  getModerators (displayName = this.channel, amountToShow = 10, search = '') {
    return new Promise((resolve, reject) => {
      ...
      webRequest(this.authKey, postData).then((result) => {
        ...
        result.errors !== undefined ? reject(new Error(result.errors['0'].message)) : resolve(result.data.userByDisplayName.chatModerators.list)
      })
    })
  }
  ...
  processBan (type, username, streamer) {
    return new Promise(async (resolve, reject) => {
      ...
      webRequest(this.authKey, postData).then((result) => {
        result.errors !== undefined ? reject(new Error(result.errors['0'].message)) : result.data[type].err !== null ? reject(result.data[type].err) : resolve(true)
      }).catch(reject)
    })
  }
}
