...
class dliveUtil {
  ...
  getDliveStreamingCategories () {
    return new Promise((resolve, reject) => {
      ...
      this.searchCategory('', 125).then((result) => { // Temporarily till i find another solution (Max categories are 125)
        resolve(result)
      }).catch(reject)
    })
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (language) {
        ...
        webRequest(this.authKey, postData).then((result) => {
          result.errors !== undefined ? reject(new Error(result.errors['0'].message)) : resolve(result.data.livestreams.list)
        }).catch(reject)
      } else {
  ...
  getRisingCreators () {
    return new Promise((resolve, reject) => {
      ...
      webRequest(this.authKey, postData).then((result) => {
        result.errors !== undefined ? reject(new Error(result.errors['0'].message)) : resolve(result.data.leaderboard.list)
      }).catch(reject)
    })
  }
  ...
  searchTerm (term, amountToShow = 10) {
    return new Promise((resolve, reject) => {
      ...
      webRequest(this.authKey, postData).then((result) => {
        let searchData = result.data.search
        let results = {}
  ...
  searchCategory (term, amountToShow = 10) {
    return new Promise((resolve, reject) => {
      ...
      webRequest(this.authKey, postData).then((result) => {
        result.errors !== undefined ? reject(new Error(result.errors['0'].message)) : resolve(result.data.search.trendingCategories.list)
      }).catch(reject)
    })
  }
}
